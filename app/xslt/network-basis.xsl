        ...
        <xsl:for-each select="$years">
            ...
            <xsl:for-each select="doc('../data/lists.xml')//listPerson/person[@xml:id]">
                ...
                <person>
                    ...
                    <count><xsl:value-of select="count(collection('../data/doc')[//origDate/substring(@when,1,4)=$year or 
                        //origDate/substring(@notBefore,1,4)=$year 
                        or //origDate/substring(@notAfter,1,4)=$year
                        or xs:string(//origDate/number(substring(@to,1,4)) 
                        - ceiling(((number(substring(@to,1,4)) - number(substring(@from,1,4))) div 2)))=$year]//body//rs[@type='name'][@key=$person-id])"/></count>
                    <status><xsl:value-of select="note[@type='ontological-status']"/></status>
                    <documents>
                        <!-- Alle Dokumente durchgehen; kommt die aktuelle Person in dem Dokument überhaupt vor? -->
                        <xsl:for-each select="collection('../data/doc')[//origDate/substring(@when,1,4)=$year or 
                            //origDate/substring(@notBefore,1,4)=$year 
                            or //origDate/substring(@notAfter,1,4)=$year
                            or xs:string(//origDate/number(substring(@to,1,4)) 
                            - ceiling(((number(substring(@to,1,4)) - number(substring(@from,1,4))) div 2)))=$year]//body[exists(.//rs[@type='name'][@key=$person-id])]">
                            <document>
                                <filename><xsl:value-of select="ancestor::TEI//idno[@type='filename']"/></filename>
                                <date when="{ancestor::TEI//origDate/@when}" from="{ancestor::TEI//origDate/@from}" 
                                    to="{ancestor::TEI//origDate/@to}" notBefore="{ancestor::TEI//origDate/@notBefore}" 
                                    notAfter="{ancestor::TEI//origDate/@notAfter}"><xsl:value-of select="ancestor::TEI//origDate"/></date>
                                <self>
                                    <id><xsl:value-of select="$person-id"/></id>
        ...
        <xsl:result-document href="network-basis.xml">
            ...
            <xsl:for-each select="doc('../data/lists.xml')//listPerson/person[@xml:id]">
                ...
                <person>
                    <id><xsl:value-of select="$person-id"/></id>
                    <name><xsl:value-of select="persName[1]"/></name>
                    <count><xsl:value-of select="count(collection('../data/doc')//body//rs[@type='name'][@key=$person-id])"/></count>
                    <status><xsl:value-of select="note[@type='ontological-status']"/></status>
                    <documents>
                        <!-- Alle Dokumente durchgehen; kommt die aktuelle Person in dem Dokument überhaupt vor? -->
                        <xsl:for-each select="collection('../data/doc')//body[exists(.//rs[@type='name'][@key=$person-id])]">
                            <document>
                                <filename><xsl:value-of select="ancestor::TEI//idno[@type='filename']"/></filename>
                                <date when="{ancestor::TEI//origDate/@when}" from="{ancestor::TEI//origDate/@from}" 
                                    to="{ancestor::TEI//origDate/@to}" notBefore="{ancestor::TEI//origDate/@notBefore}" 
